...
let CrmService = CrmService_1 = class CrmService {
    constructor(behaviorRepo, alertRepo) {
    ...
    async track(dto) {
        try {
            const event = this.behaviorRepo.create({
                ...
                severity: dto.severity ?? user_behavior_event_entity_1.EventSeverity.INFO,
    ...
    async getDashboardStats(days = 7) {
        ...
        const [totalEvents, errorEvents, uniqueSessions, openAlerts, criticalAlerts,] = await Promise.all([
            this.behaviorRepo.count({ where: { created_at: (0, typeorm_2.MoreThan)(since) } }),
            this.behaviorRepo.count({
                where: { severity: user_behavior_event_entity_1.EventSeverity.ERROR, created_at: (0, typeorm_2.MoreThan)(since) },
            }),
            this.behaviorRepo
            ...
            this.alertRepo.count({ where: { status: crm_ai_alert_entity_1.AlertStatus.OPEN } }),
            this.alertRepo.count({
                where: { status: crm_ai_alert_entity_1.AlertStatus.OPEN, severity: crm_ai_alert_entity_1.AlertSeverity.CRITICAL },
            }),
        ]);
        ...
        const avgResponseTime = await this.behaviorRepo
            ...
            .where('e.event_type = :t', { t: user_behavior_event_entity_1.BehaviorEventType.API_CALL })
        ...
        const topPages = await this.behaviorRepo
            ...
            .addSelect('COUNT(*)', 'count')
            .where('e.event_type = :t', { t: user_behavior_event_entity_1.BehaviorEventType.PAGE_VIEW })
            .andWhere('e.created_at > :since', { since })
    ...
    async getDailyTrend(days) {
        ...
        for (let i = days - 1; i >= 0; i--) {
            ...
            const [total, errors, bookings, payments] = await Promise.all([
                this.behaviorRepo.count({ where: { created_at: (0, typeorm_2.Between)(start, end) } }),
                this.behaviorRepo.count({
                    where: { severity: user_behavior_event_entity_1.EventSeverity.ERROR, created_at: (0, typeorm_2.Between)(start, end) },
                }),
                this.behaviorRepo.count({
                    where: { event_type: user_behavior_event_entity_1.BehaviorEventType.BOOKING_COMPLETE, created_at: (0, typeorm_2.Between)(start, end) },
                }),
                this.behaviorRepo.count({
                    where: { event_type: user_behavior_event_entity_1.BehaviorEventType.PAYMENT_SUCCESS, created_at: (0, typeorm_2.Between)(start, end) },
                }),
            ]);
            trend.push({
    ...
    // ═══════════════════════════════════════════════════════════════
    async getUserBehaviorSummary(userId, days = 30) {
        const since = new Date(Date.now() - days * 86400_000);
        const events = await this.behaviorRepo.find({
            where: { user_id: userId, created_at: (0, typeorm_2.MoreThan)(since) },
            order: { created_at: 'DESC' },
        });
        const summary = {
            totalActions: events.length,
            pageViews: events.filter(e => e.event_type === user_behavior_event_entity_1.BehaviorEventType.PAGE_VIEW).length,
            bookingStarts: events.filter(e => e.event_type === user_behavior_event_entity_1.BehaviorEventType.BOOKING_START).length,
            bookingCompletes: events.filter(e => e.event_type === user_behavior_event_entity_1.BehaviorEventType.BOOKING_COMPLETE).length,
            bookingAbandons: events.filter(e => e.event_type === user_behavior_event_entity_1.BehaviorEventType.BOOKING_ABANDON).length,
            paymentSuccesses: events.filter(e => e.event_type === user_behavior_event_entity_1.BehaviorEventType.PAYMENT_SUCCESS).length,
            paymentFails: events.filter(e => e.event_type === user_behavior_event_entity_1.BehaviorEventType.PAYMENT_FAIL).length,
            errors: events.filter(e => e.severity === user_behavior_event_entity_1.EventSeverity.ERROR).length,
        ...
        const pageCounts = {};
        events
            .filter(e => e.event_type === user_behavior_event_entity_1.BehaviorEventType.PAGE_VIEW && e.page_path)
    ...
    // ═══════════════════════════════════════════════════════════════
    async analyzeWithAI(windowHours = 24) {
        const since = new Date(Date.now() - windowHours * 3600_000);
        // جمع‌آوری داده‌های خام
        const [totalEvents, errorEvents, paymentFails, bookingAbandons, apiErrors,] = await Promise.all([
            this.behaviorRepo.count({ where: { created_at: (0, typeorm_2.MoreThan)(since) } }),
            this.behaviorRepo.count({ where: { severity: user_behavior_event_entity_1.EventSeverity.ERROR, created_at: (0, typeorm_2.MoreThan)(since) } }),
            this.behaviorRepo.count({ where: { event_type: user_behavior_event_entity_1.BehaviorEventType.PAYMENT_FAIL, created_at: (0, typeorm_2.MoreThan)(since) } }),
            this.behaviorRepo.count({ where: { event_type: user_behavior_event_entity_1.BehaviorEventType.BOOKING_ABANDON, created_at: (0, typeorm_2.MoreThan)(since) } }),
            this.behaviorRepo.count({ where: { event_type: user_behavior_event_entity_1.BehaviorEventType.API_ERROR, created_at: (0, typeorm_2.MoreThan)(since) } }),
        ]);
...
`.trim();
        try {
            ...
            const alert = this.alertRepo.create({
                ...
                status: crm_ai_alert_entity_1.AlertStatus.OPEN,
                title: result.title,
    ...
    async cleanOldEvents() {
        ...
        const result = await this.behaviorRepo.delete({ created_at: (0, typeorm_2.LessThan)(ninetyDaysAgo) });
        this.logger.log(`🧹 ${result.affected} رویداد قدیمی حذف شد`);
    }
    // ═══════════════════════════════════════════════════════════════
    // ۶. مدیریت هشدارها
    // ═══════════════════════════════════════════════════════════════
    async getAlerts(status, page = 1, limit = 20) {
    ...
    async updateAlertStatus(id, status, adminId, note) {
        const alert = await this.alertRepo.findOne({ where: { id } });
        if (!alert)
            throw new Error('هشدار یافت نشد');
        alert.status = status;
    ...
    async getUserEvents(userId, page = 1, limit = 50) {
...
], CrmService.prototype, "scheduledAiCheck", null);
__decorate([
    (0, schedule_1.Cron)(schedule_1.CronExpression.EVERY_DAY_AT_3AM),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], CrmService.prototype, "cleanOldEvents", null);
exports.CrmService = CrmService = CrmService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(user_behavior_event_entity_1.UserBehaviorEvent)),
    __param(1, (0, typeorm_1.InjectRepository)(crm_ai_alert_entity_1.CrmAiAlert)),
