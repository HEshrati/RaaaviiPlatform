...
let PsychometricService = class PsychometricService {
    ...
    buildProfile(userId, testResults) {
        ...
        for (const r of testResults) {
            ...
            if (id.includes('phq9') || id.includes('phq_9')) {
                ...
                if (profile.phq9 === 0 && r.total_score)
                    profile.phq9 = r.total_score;
            }
            // GAD-7
            if (id.includes('gad7') || id.includes('gad_7')) {
                profile.gad7 = this.extract(s, ['total', 'score'], 0);
                if (profile.gad7 === 0 && r.total_score)
                    profile.gad7 = r.total_score;
            }
            // Love Languages
            if (id.includes('love')) {
    ...
    detectRedFlags(p) {
        const flags = [];
        if (p.phq9 !== null && p.phq9 >= 15)
            flags.push('افسردگی شدید');
        if (p.phq9 !== null && p.phq9 >= 20)
            flags.push('افسردگی بحرانی');
        if (p.gad7 !== null && p.gad7 >= 15)
            flags.push('اضطراب شدید');
        if (p.ecr?.style === 'disorganized')
            flags.push('دلبستگی بی‌سازمان');
        if (p.gottman && p.gottman.toxic > 16)
            flags.push('الگوهای ارتباطی مخرب');
        if (p.neo && p.neo.N > 24)
            flags.push('روان‌رنجوری بالا');
        return flags;
    }
    calcMentalHealthScore(p) {
        let score = 80;
        if (p.phq9 !== null)
            score -= Math.min(30, p.phq9 * 1.5);
        if (p.gad7 !== null)
            score -= Math.min(20, p.gad7 * 1.2);
        if (p.neo)
            score -= Math.max(0, (p.neo.N - 15) * 0.8);
        if (p.ecr?.style === 'disorganized')
            score -= 15;
        if (p.ecr?.style === 'anxious')
            score -= 8;
        return Math.max(0, Math.min(100, Math.round(score)));
    }
    calcRelationshipReadiness(p) {
        let score = 60;
        if (p.ecr?.style === 'secure')
            score += 20;
        if (p.ecr?.style === 'anxious')
            score -= 10;
        if (p.ecr?.style === 'avoidant')
            score -= 15;
        if (p.ecr?.style === 'disorganized')
            score -= 25;
        if (p.erq && p.erq.reappraisal > 21)
            score += 10;
        if (p.iri && p.iri.empathy > 15)
            score += 8;
        if (p.gottman && p.gottman.healthy > p.gottman.toxic)
            score += 10;
        if (p.neo && p.neo.A > 18)
            score += 5;
        if (p.redFlags.length > 0)
            score -= p.redFlags.length * 8;
        return Math.max(0, Math.min(100, Math.round(score)));
    }
    extract(s, keys, def) {
        for (const k of keys) {
            if (s[k] !== undefined && s[k] !== null)
                return Number(s[k]);
            if (s[k.toLowerCase()] !== undefined)
                return Number(s[k.toLowerCase()]);
        }
        return def;
    }
};
