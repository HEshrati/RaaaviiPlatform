...
exports.IntelligenceService = exports.TEST_PHASES = void 0;
/**
 * ============================================================
...
let IntelligenceService = IntelligenceService_1 = class IntelligenceService {
    ...
    `, [userId]);
        // ── بررسی فازها ──────────────────────────
        const doneTests = results.map((r) => r.test_name);
        const phase1Done = exports.TEST_PHASES[1].tests.filter(t => doneTests.includes(t)).length;
        const phase2Done = exports.TEST_PHASES[2].tests.filter(t => doneTests.includes(t)).length;
        const phase3Done = exports.TEST_PHASES[3].tests.filter(t => doneTests.includes(t)).length;
        const phase1Complete = phase1Done >= exports.TEST_PHASES[1].tests.length;
        const phase2Complete = phase2Done >= exports.TEST_PHASES[2].tests.length;
        const profileCompleteness = Math.round((phase1Done * 8 + phase2Done * 5 + phase3Done * 2) / 65 * 100);
        // ── محاسبه کتگوری‌های مقاله پیشنهادی ──
        const artCats = new Set();
        for (const r of results) {
            const cats = TEST_ARTICLE_CATS[r.test_name] || [];
            cats.forEach((c) => artCats.add(c));
            // بر اساس نتیجه تست
            const scores = typeof r.scores === 'string' ? JSON.parse(r.scores) : r.scores || {};
            if (r.test_name === 'neo_ffi') {
                if ((scores.N || 0) > 18) {
                    artCats.add('مدیریت استرس');
                    artCats.add('بهداشت روان');
                }
                if ((scores.A || 0) < 14)
                    artCats.add('روابط سالم');
                if ((scores.O || 0) > 20)
                    artCats.add('رشد فردی');
            }
            if (r.test_name === 'ecr_r') {
                ...
                if ((scores.AVO || 0) > 38)
                    artCats.add('روابط سالم');
                if ((scores.ANX || 0) <= 27 && (scores.AVO || 0) <= 27)
                    artCats.add('روانشناسی مثبت');
            }
        ...
        for (const r of results) {
            ...
            if (typeMap) {
                ...
                types.forEach((t) => evTypes.add(t));
            }
        }
        // فال‌بک
        if (evTypes.size === 0)
            ['دورهمی', 'گفتگو', 'هنر'].forEach(t => evTypes.add(t));
    ...
    `, [userId, phase1Complete, phase2Complete, phase3Done,
        ...
        for (const r of results) {
            const phase = Object.entries(exports.TEST_PHASES).find(([, p]) => p.tests.includes(r.test_name))?.[0] || '3';
            await this.ds.query(`
    ...
    // ══════════════════════════════════════════
    async validateRecommendations(userId, artCats, evTypes) {
        const issues = [];
        // چک مقالات
    ...
    async getFullProfile(userId) {
      ...
      `, [userId]),
        ...
        const profile = sp[0] || {};
        const doneIds = progress.map((p) => p.test_id);
        // وضعیت هر فاز
        const phases = Object.entries(exports.TEST_PHASES).map(([num, ph]) => ({
            phase: parseInt(num),
            label: ph.label,
            ...
            tests: ph.tests.map(t => ({
                ...
                result: tests.find((r) => r.test_name === t)?.main_result || null,
                unlockedAt: progress.find((p) => p.test_id === t)?.unlocked_at || null,
            })),
    ...
    // ══════════════════════════════════════════
    async getArticleRecommendations(userId, limit = 10) {
        const sp = await this.ds.query('SELECT recommended_article_cats FROM smart_profiles WHERE user_id=$1', [userId]);
    ...
    // ══════════════════════════════════════════
    async getEventRecommendations(userId, limit = 6) {
        const sp = await this.ds.query('SELECT recommended_event_types,extroversion_score,location_preference FROM smart_profiles WHERE user_id=$1', [userId]);
      ...
      LIMIT $2
    `, [evTypes, limit]).catch(() => this.ds.query('SELECT * FROM events WHERE start_date>NOW() ORDER BY start_date LIMIT $1', [limit]));
    ...
    // ══════════════════════════════════════════
    async getNextRecommendedTests(userId) {
        const profile = await this.getFullProfile(userId);
        const doneIds = profile.phases.flatMap((p) => p.tests.filter((t) => t.done).map((t) => t.id));
        const recs = [];
        for (const phase of profile.phases) {
            if (!phase.unlocked)
                continue;
            const pending = phase.tests.filter((t) => !t.done);
            recs.push(...pending.map((t) => ({
                testId: t.id,
                phase: phase.phase,
...
], IntelligenceService.prototype, "cronSyncAll", null);
