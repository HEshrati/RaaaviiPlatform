...
exports.QUESTION_META = void 0;
exports.buildAnswerFingerprint = buildAnswerFingerprint;
exports.cosineSimilarity = cosineSimilarity;
exports.idealCompatibility = idealCompatibility;
exports.getFacetInsights = getFacetInsights;
// ── کاتالوگ کامل سوالات با معنای روانشناختی ──────────────────
exports.QUESTION_META = {
    neo_ffi: [
        // E — Extraversion
...
function buildAnswerFingerprint(testId, scores) {
    const fingerprint = {};
    const answers = scores?.answers || {};
    const meta = exports.QUESTION_META[testId];
...
function buildFromSubscales(testId, scores) {
    const fp = {};
    if (!scores)
        return fp;
    // NEO
    ...
    // ECR
    if (testId === 'ecr_r') {
        if (scores.ANX !== undefined)
            fp.ANX = scores.ANX;
        if (scores.AVO !== undefined)
            fp.AVO = scores.AVO;
    }
    // ERQ
    if (testId === 'erq') {
        if (scores.CR !== undefined)
            fp.CR = scores.CR;
        if (scores.ES !== undefined)
            fp.ES = scores.ES;
    }
    return fp;
}
...
function idealCompatibility(testId, a, b) {
    const details = [];
    let score = 0;
    let count = 0;
    if (testId === 'neo_ffi') {
        ...
        if (nA < 12 && nB < 12)
            details.push('✅ روان‌رنجوری پایین در هر دو');
        else if (nA > 20 || nB > 20)
            details.push('⚠️ روان‌رنجوری بالا');
        score += nScore * 0.3;
        count++;
        ...
        if (aMean > 20)
            details.push('✅ توافق‌پذیری بالا');
        score += (aScore + aBonus) * 0.25;
        count++;
        // C: شباهت (0.15 وزن)
        ...
        if (eDiff > 5)
            details.push('✅ ترکیب درون+برون‌گرا');
        score += eScore * 0.15;
        count++;
        // O: شباهت (0.15 وزن)
    ...
    if (testId === 'ecr_r') {
        ...
        if (anxA > 36 && avoB > 36) {
            score += 25;
            details.push('⚠️ اضطرابی+اجتنابی');
        }
        else {
            score += (anxScore * 0.6 + avoScore * 0.4);
        }
        count++;
