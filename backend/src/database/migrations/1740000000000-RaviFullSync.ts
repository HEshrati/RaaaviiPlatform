...
export class RaviFullSync1740000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // ستون‌های legacy date/time در همه‌ی نصب‌ها وجود ندارند؛ ابتدا schema را
    // بررسی می‌کنیم تا خطای PostgreSQL تراکنش migration را abort نکند.
    if (await queryRunner.hasColumn('events', 'date')) {
      const hasTime = await queryRunner.hasColumn('events', 'time');
      await queryRunner.query(`
        UPDATE "events"
          SET "start_date" = ("date" || ' ' || ${hasTime ? 'COALESCE("time", \'00:00\')' : "'00:00'"})::timestamp
    ...
    if (await queryRunner.hasColumn('smart_profiles', 'introvert_score')) {
      await queryRunner.query(`
    ...
    if (await queryRunner.hasColumn('smart_profiles', 'total_events_registered')) {
      await queryRunner.query(`
        UPDATE "smart_profiles"
    ...
    await queryRunner.query(`
      ...
      -- notifications در نصب‌های قدیمی userId/read دارد، نه user_id/is_read؛
      -- بنابراین index ناسازگار آن عمداً در این migration ساخته نمی‌شود.
