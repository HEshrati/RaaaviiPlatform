...
const ARTICLE_PLAN: { topic:string; group:string; author:string; angle:string; emoji:string; category:string; tags:string[]; unsplash_id:string; recommendedFor:string[] }[] = [
...
// نگاشت دسته‌بندی‌های فارسی و انگلیسی به پوشه‌ی عکس‌های لوکال (fallback)
const IMAGE_CATEGORY_MAP: Record<string, string> = {
  'مدیریت استرس': 'stress-management', stress: 'stress-management',
  'هوش هیجانی': 'emotional-intelligence', emotion: 'emotional-intelligence',
  'رشد فردی': 'personal-growth', growth: 'personal-growth',
  'بهداشت روان': 'mental-health', health: 'mental-health', psychology: 'mental-health',
  'روابط سالم': 'healthy-relationships', relationship: 'healthy-relationships',
  attachment: 'healthy-relationships', communication: 'healthy-relationships', social: 'healthy-relationships',
  'ذهن\u200cآگاهی': 'mindfulness', mindfulness: 'mindfulness',
  'روانشناسی مثبت': 'positive-psychology',
  خودشناسی: 'self-awareness',
};
const LOCAL_IMAGE_COUNT = 4; // تعداد عکس موجود برای هر دسته در uploads/articles
...
function localFallbackImage(category: string, seed: string): string {
  const folder = IMAGE_CATEGORY_MAP[category] || 'mental-health';
  const idx = (hashSeed(seed) % LOCAL_IMAGE_COUNT) + 1;
  return `/uploads/articles/${folder}-${idx}.jpg`;
}

/**
 * ساخت image_url برای مقاله: ابتدا تلاش می‌کند loremflickr را با یک HEAD سریع چک کند
 * (مهلت کوتاه). اگر در دسترس بود، همان URL مستقیم لورم‌فلیکر برگردانده می‌شود (رفتار قبلی).
 * اگر timeout/خطا داد، به یکی از عکس‌های لوکال همان دسته (fallback) سوییچ می‌کند تا مقاله
 * هیچ‌وقت بدون عکس قابل‌نمایش نماند.
 */
async function buildArticleImageUrl(category: string, seed: string, keyword?: string): Promise<string> {
  const kw = keyword || (/^[a-zA-Z,\s]+$/.test(category) ? category : 'psychology');
  const lock = hashSeed(seed) % 500;
  const remoteUrl = `https://loremflickr.com/800/400/${encodeURIComponent(kw)}?lock=${lock}`;

  try {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 5000);
    const res = await fetch(remoteUrl, { method: 'HEAD', signal: ctrl.signal });
    clearTimeout(timer);
    if (res.ok || res.status === 302 || res.status === 301) {
      return remoteUrl;
    }
    throw new Error(`loremflickr status ${res.status}`);
  } catch {
    return localFallbackImage(category, seed);
  }
}
...
export class AiContentService {
  ...
  constructor(
    @InjectRepository(AiContent) private readonly contentRepo: Repository<AiContent>,
    @InjectDataSource() public readonly dataSource: DataSource,
  ) {}

  @Cron('30 4 * * *') // 8:00 صبح ایران
  async dailyArticleGeneration(): Promise<void> {
...
Write in Persian directly:`}]
            ...
            body = (data as any).choices?.[0]?.message?.content || '';
          } catch (fetchErr: any) {
        ...
        const emMap:any={روابط_سالم:'💞','بهداشت روان':'🌿','هوش هیجانی':'💡','رشد فردی':'🚀','مدیریت استرس':'🧘','ذهن‌آگاهی':'🌸','روانشناسی مثبت':'✨','خودشناسی':'🔍'};
        const clMap:any={'روابط سالم':'#f97316','بهداشت روان':'#22c55e','هوش هیجانی':'#0ea5e9','رشد فردی':'#ec4899','مدیریت استرس':'#FF6B00','ذهن‌آگاهی':'#10b981','روانشناسی مثبت':'#a855f7','خودشناسی':'#eab308'};

        const imageUrl = await buildArticleImageUrl(cat, slug, `${cat},psychology`);
  ...
  async generateBulk50(): Promise<{ created: number; skipped: number; errors: number }> {
    let created = 0, skipped = 0, errors = 0;
    this.logger.log('Starting bulk 50 generation');
    for (const plan of ARTICLE_PLAN) {
      try {
        const exists = await this.contentRepo.findOne({ where: { topic_group: plan.group, reference_author: plan.author } as any });
        if (exists) { skipped++; continue; }
        const art = await this.generateFromPlan(plan);
        if (art) created++;
        await new Promise(r => setTimeout(r, 2000));
      } catch (e) { this.logger.error(`Bulk error: ${e.message}`); errors++; }
    }
    return { created, skipped, errors };
  }

  async generateFromPlan(plan: typeof ARTICLE_PLAN[0]): Promise<AiContent | null> {
    ...
    const slugExists = await this.contentRepo.findOne({ where: { slug } as any });
    const imageUrl = await buildArticleImageUrl(plan.category || plan.group, slug, plan.angle || plan.group || "psychology");
    const article = this.contentRepo.create({
      title: content.title, content: content.body, summary: content.summary,
      tags: content.tags, status: ContentStatus.PUBLISHED,
    ...
    return this.contentRepo.save(article) as any;
  }

  private async getUnderrepresentedTopics() {
    const rows = await this.dataSource.query(
      "SELECT topic_group, reference_author FROM ai_content WHERE status='published' AND topic_group IS NOT NULL"
    ).catch(() => []);
    const done = new Set(rows.map((r: any) => `${r.topic_group}|||${r.reference_author}`));
    return ARTICLE_PLAN.filter(p => !done.has(`${p.group}|||${p.author}`));
  }

  private async callAIForArticle(plan: typeof ARTICLE_PLAN[0]): Promise<{ title: string; body: string; summary: string; tags: string[] } | null> {
    if (!AI_API_KEY) {
...
{"title":"عنوان جذاب فارسی","summary":"خلاصه ۲-۳ جمله","tags":["تگ۱","تگ۲","تگ۳"],"body":"متن کامل"}`;
    ...
    try {
      ...
      if (!res.ok) { this.logger.warn(`AI ${res.status}`); return null; }
      const data = await res.json();
      const raw = ((data as any).choices?.[0]?.message?.content || '').replace(/```json\n?/g,'').replace(/```\n?/g,'').trim();
      return JSON.parse(raw);
    } catch (e) { this.logger.error(`AI error: ${e.message}`); return null; }
  }

  private placeholder(plan: typeof ARTICLE_PLAN[0]): string {
  ...
  async getPublishedArticles(page = 1, limit = 20, category?: string): Promise<AiContent[]> {
    return this.findPublished(limit, (page - 1) * limit, category);
  }

  async findPublished(limit = 20, offset = 0, category?: string): Promise<AiContent[]> {
    const qb = this.contentRepo.createQueryBuilder('a')
      .where('a.status = :s', { s: ContentStatus.PUBLISHED })
      .orderBy('a.published_at', 'DESC').take(limit).skip(offset);
    if (category) qb.andWhere('a.category = :c', { c: category });
    return qb.getMany();
  }

  async findRelated(id: string, topicGroup: string, limit = 3): Promise<AiContent[]> {
  ...
  async findOne(id: string): Promise<AiContent> {
    const a = await this.contentRepo.findOne({ where: { id } as any });
    if (!a) throw new NotFoundException('مقاله یافت نشد');
    await this.contentRepo.update(id, { view_count: () => 'view_count + 1' } as any);
    return a;
  }

  async getPersonalized(testResults: any[]): Promise<AiContent[]> {
    const cats = this.getRecsFromTests(testResults);
    if (!cats.length) return this.findPublished(10);
    const rows = await this.dataSource.query(
      "SELECT * FROM ai_content WHERE status='published' AND recommended_for::jsonb ?| $1 ORDER BY published_at DESC LIMIT 10",
      [cats]
    ).catch(() => []);
    return rows.length ? rows : this.findPublished(10);
  }

  private getRecsFromTests(results: any[]): string[] {
  ...
  async getDrafts(): Promise<AiContent[]> {
    return this.contentRepo.find({ where: { status: ContentStatus.DRAFT } as any, order: { created_at: 'DESC' } });
  }

  async approveAndPublish(id: string, adminId?: string): Promise<AiContent> {
    await this.contentRepo.update(id, { status: ContentStatus.PUBLISHED, published_at: new Date(), reviewed_by: adminId } as any);
    return this.findOne(id);
  }

  async rejectContent(id: string, reason?: string): Promise<AiContent> {
    await this.contentRepo.update(id, { status: ContentStatus.REJECTED, admin_note: reason } as any);
    return this.findOne(id);
  }

  async editContent(id: string, body: any): Promise<AiContent> {
    await this.contentRepo.update(id, body as any);
    return this.findOne(id);
  }

  async answerSupportQuestion(question: string): Promise<{ answer: string }> {
    if (!AI_API_KEY) return { answer: 'سرویس پشتیبانی در دسترس نیست.' };
    try {
      const res = await fetch(AI_API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${AI_API_KEY}` },
      ...
      const data = await res.json();
      return { answer: (data as any).choices?.[0]?.message?.content || 'پاسخی یافت نشد.' };
    } catch { return { answer: 'خطا در پردازش سوال.' }; }
  }
  ...
  async generatePsychologicalArticle(topicOverride?: string, keywords?: string[], wordCount?: number): Promise<any> {
    ...
    const resp = await fetch('https://api.gapgpt.app/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content:
          `Write a comprehensive psychology article in PERSIAN (Farsi) language.
Topic: ${topic}
...
Start writing the Persian article immediately without any introduction:`
    ...
    if (body.split(' ').length < 500) {
      ...
      if (body2.split(' ').length > body.split(' ').length) body = body2;
    }
    ...
    const emMap:any = {'روابط سالم':'💞','بهداشت روان':'🌿','هوش هیجانی':'💡','رشد فردی':'🚀','مدیریت استرس':'🧘','ذهن‌آگاهی':'🌸','روانشناسی مثبت':'✨','خودشناسی':'🔍'};
    const clMap:any = {'روابط سالم':'#f97316','بهداشت روان':'#22c55e','هوش هیجانی':'#0ea5e9','رشد فردی':'#ec4899','مدیریت استرس':'#FF6B00','ذهن‌آگاهی':'#10b981','روانشناسی مثبت':'#a855f7','خودشناسی':'#eab308'};
    ...
    const imageUrl = await buildArticleImageUrl(category, slug, `${cat},mind`);
