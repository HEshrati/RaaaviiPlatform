...
export function getBaleWebhookSecret(): string {
  try {
    return readFileSync(join(process.cwd(), 'uploads', '.bale-webhook-secret'), 'utf8').trim()
      || process.env.BALE_BOT_WEBHOOK_SECRET || '';
  } catch {
    return process.env.BALE_BOT_WEBHOOK_SECRET || '';
  }
}
...
export class BaleBotService {
  ...
  constructor(@InjectDataSource() private readonly ds: DataSource) {
    ...
    if (this.enabled && this.token && secret) {
      const url = `${this.siteUrl}/api/bale/webhook/${secret}`;
      this.setWebhook(url).then(r => {
        if (r?.result) this.logger.log('✅ Bale webhook configured');
      }).catch(() => {});
    } else if (this.enabled && (!this.token || !secret)) {
      this.logger.error('Bale bot is enabled but token or webhook secret is missing');
    }
  }
  ...
  private async getChatId(phone: string): Promise<bigint | null> {
    const norm = this.normalizePhone(phone);
    const rows = await this.ds.query(
      'SELECT chat_id FROM bale_user_chats WHERE phone=$1 LIMIT 1', [norm]
    );
    return rows?.[0]?.chat_id || null;
  }

  private async getPhoneByChat(chatId: any): Promise<string | null> {
    const rows = await this.ds.query(
      'SELECT phone FROM bale_user_chats WHERE chat_id=$1::bigint LIMIT 1',
      [String(chatId)]
    );
    return rows?.[0]?.phone || null;
  }

  private async getUserByChatId(chatId: any) {
    const rows = await this.ds.query(`
      SELECT u.id, u.name, u.phone_number as phone
      FROM bale_user_chats b
      JOIN users u ON u.phone_number = '0' || SUBSTRING(b.phone, 3)
  ...
  // ── ارسال پیام ─────────────────────────────────────────────────────
  async sendMessage(chatId: any, text: string, extra: any = {}): Promise<boolean> {
    ...
    try {
      ...
      if (!data.ok) this.logger.warn(`Bale send failed: ${JSON.stringify(data)}`);
      return !!data.ok;
    } catch (e: any) {
  ...
  private async sendToPhone(phone: string, text: string, extra?: any): Promise<boolean> {
  ...
  // ── منوی اصلی ──────────────────────────────────────────────────────
  async showMainMenu(chatId: any): Promise<void> {
  ...
  // ── لیست رویدادها ──────────────────────────────────────────────────
  private async sendEventsList(chatId: any): Promise<void> {
    ...
    const buttons = events.map((e: any) => [
  ...
  // ── لیست روانشناسان ────────────────────────────────────────────────
  private async sendPsychologistsList(chatId: any): Promise<void> {
    ...
    if (!therapists?.length) {
      await this.sendMessage(chatId,
        '🏥 *روانشناسان راوی*\n\n' +
        'برای مشاهده لیست کامل روانشناسان به سایت راوی مراجعه کنید.',
  ...
  // ── لیست تست‌ها ────────────────────────────────────────────────────
  private async sendTestsList(chatId: any): Promise<void> {
    ...
    const done = await this.ds.query(
      'SELECT DISTINCT test_name FROM test_results WHERE user_id=$1', [user.id]
    ).catch(() => []);
    const doneSet = new Set(done.map((r: any) => r.test_name));
  ...
  // ── رزروهای من ─────────────────────────────────────────────────────
  private async sendBookingsList(chatId: any): Promise<void> {
    const user = await this.getUserByChatId(chatId);
    if (!user) {
      await this.sendMessage(chatId, '❌ ابتدا وارد سایت شوید.');
      return;
    ...
    bookings.forEach((b: any, i: number) => {
      ...
      if (b.status === 'cancelled') {
        ...
        if (reason) text += `📝 دلیل لغو: ${reason}\n`;
        else text += `📝 لغو توسط کاربر\n`;
      }
  ...
  // ── سایر send methods ───────────────────────────────────────────────
  async sendEventRecommendations(phone: string, events: any[]): Promise<boolean> {
    ...
    events.slice(0, 3).forEach((e, i) => {
      ...
      if (e.price) text += `💰 ${Number(e.price/10).toLocaleString()} تومان\n`;
      text += `[رزرو](${this.siteUrl}/events/${e.id})\n\n`;
    });
    return this.sendToPhone(phone, text);
  }

  async sendPsychologistRecommendations(phone: string, therapists: any[]): Promise<boolean> {
    if (!therapists?.length) return false;
  ...
  async sendEventReminder(phone: string, booking: any): Promise<boolean> {
    const text =
      '🗓️ *یادآوری رویداد*\n\n' +
      `*${booking.title}*\n` +
  ...
  async sendAbandonedPaymentReminder(phone: string, booking: any): Promise<boolean> {
    const text =
      '⚠️ *پرداخت ناتمام*\n\n' +
  ...
  async sendCapacityReachedNotification(phone: string, event: any): Promise<boolean> {
    const text =
      '🎉 *رویداد به حد نصاب رسید!*\n\n' +
      `*${event.title}*\n` +
  ...
  async sendPaymentReceipt(phone: string, payment: any): Promise<boolean> {
  ...
  async sendGroupMatchResult(phone: string, group: any, event: any): Promise<boolean> {
  ...
  async handleUpdate(update: any): Promise<void> {
    ...
    if (update?.callback_query) {
      ...
      if (data === 'events')       { await this.sendEventsList(chatId); return; }
      if (data === 'psychologists'){ await this.sendPsychologistsList(chatId); return; }
      if (data === 'tests')        { await this.sendTestsList(chatId); return; }
      if (data === 'bookings')     { await this.sendBookingsList(chatId); return; }
      if (data === 'status')       { await this.sendStatus(chatId); return; }
      if (data === 'menu')         { await this.showMainMenu(chatId); return; }
      return;
    }
    ...
    if (msg.contact?.phone_number) {
      ...
      await this.ds.query(
        `INSERT INTO bale_user_chats (phone, chat_id, first_name, last_name, updated_at)
         VALUES ($1, $2::bigint, $3, $4, NOW())
         ON CONFLICT (phone) DO UPDATE
         SET chat_id=$2::bigint, first_name=$3, last_name=$4, updated_at=NOW()`,
        [phone, String(chatId), firstName, lastName]
      );

      await this.sendMessage(chatId,
        '✅ *شماره شما ثبت شد!*\n\n' +
        'از این پس کدهای OTP و اعلان‌ها اینجا می‌رسند.',
    ...
    if (text.startsWith('/start')) {
      ...
      if (param.startsWith('otp_')) {
        ...
        if (phone?.length >= 10) {
          ...
          await this.ds.query(
            `INSERT INTO bale_user_chats (phone, chat_id, updated_at)
             VALUES ($1, $2::bigint, NOW())
             ON CONFLICT (phone) DO UPDATE SET chat_id=$2::bigint, updated_at=NOW()`,
            [normPhone, String(chatId)]
          );
          const otpRow = await this.ds.query(
            'SELECT code FROM otps WHERE mobile_number=$1 ORDER BY id DESC LIMIT 1', [phone]
          ).catch(() => []);
      ...
      if (param.startsWith('event_GROUP_')) {
        ...
        if (!phone) {
          await this.sendMessage(chatId,
            '❌ ابتدا شماره موبایل خود را به بات ارسال کنید.',
            { reply_markup: { keyboard: [[{ text: '📱 ارسال شماره من', request_contact: true }]], resize_keyboard: true, one_time_keyboard: true } }
          );
          return;
        }
        // چک رزرو معتبر
        ...
        const evInfo = await this.ds.query(
          'SELECT title, end_date, bale_group_link FROM events WHERE id=$1', [eventId]
        );
        const ev = evInfo?.[0];

        if (ev?.bale_group_link) {
          await this.sendMessage(chatId,
            `🎉 *لینک گروه بله آماده‌ست!*
...
` +
  ...
  private async sendStatus(chatId: any): Promise<void> {
    const phone = await this.getPhoneByChat(chatId);
    if (!phone) {
    ...
    const testCount = user ? (await this.ds.query(
      'SELECT COUNT(DISTINCT test_name) as c FROM test_results WHERE user_id=$1', [user.id]
    ).catch(() => [{ c: 0 }]))[0]?.c || 0 : 0;
    ...
    await this.sendMessage(chatId,
      ...
      `[ورود به پنل](${this.siteUrl}/dashboard)`,
      { reply_markup: { inline_keyboard: [[
        { text: '📋 منو', callback_data: 'menu' }
      ]] }}
    );
  ...
  async notifyGroups(eventId: string, groups: any[]): Promise<void> {
    for (const group of groups) {
      for (const userId of (group.memberIds || [])) {
        const rows = await this.ds.query(
          'SELECT b.phone FROM bale_user_chats b JOIN users u ON u.id=$1 WHERE b.chat_id IS NOT NULL LIMIT 1',
          [userId]
        ).catch(() => []);
        if (rows?.[0]?.phone) {
  ...
  async notifyAdmin(message: string): Promise<void> {
    const admins = await this.ds.query(
      "SELECT b.chat_id FROM bale_user_chats b JOIN users u ON u.id = b.user_id WHERE u.role='admin'"
    ).catch(() => []);
