...
export class AiChatService {
  constructor(
    @InjectRepository(TestResult) private testRepo: Repository<TestResult>,
    @InjectRepository(Profile) private profileRepo: Repository<Profile>,
    @InjectRepository(UserBehaviorEvent) private behaviorRepo: Repository<UserBehaviorEvent>,
    @InjectRepository(CrmAiAlert) private alertRepo: Repository<CrmAiAlert>,
    @InjectRepository(User) private userRepo: Repository<User>,
  ) {}

  async userChat(userId: string, messages: { role: string; content: string }[]) {
    ...
    // تفسیر هوشمند scores
    function interpretScore(name: string, s: any): string {
      const toP = (v: any, mx: number) => v != null ? Math.round(Number(v)/mx*100) : 0;
      if (name === 'neo_ffi') {
  ...
  تخیل: ${FS}/28 (${toP(FS,28)}٪) | پریشانی: ${PD}/28 (${toP(PD,28)}٪)`;
      ...
      if (name === 'phq9') return `🌿 افسردگی PHQ9: ${s.total||0}/27 - ${(s.total||0)>=20?'شدید':(s.total||0)>=15?'متوسط-شدید':(s.total||0)>=10?'متوسط':(s.total||0)>=5?'خفیف':'حداقل'}`;
      if (name === 'gad7') return `😰 اضطراب GAD7: ${s.total||0}/21 - ${(s.total||0)>=15?'شدید':(s.total||0)>=10?'متوسط':(s.total||0)>=5?'خفیف':'حداقل'}`;
      if (name.includes('matching_basis')||name==='mbti') return `🧠 MBTI: ${s.EI>=0?'E':'I'}${s.SN>=0?'N':'S'}${s.TF>=0?'F':'T'}${s.JP>=0?'P':'J'}`;
...
۴. فارسی، گرم و علمی باشد`;
  ...
  async adminChat(messages: { role: string; content: string }[]) {
    const [totalUsers, totalTests, openAlerts, recentAlerts] = await Promise.all([
      this.userRepo.count(),
      this.testRepo.count(),
      this.alertRepo.count({ where: { status: 'open' as any } }),
      this.alertRepo.find({ where: { status: 'open' as any }, order: { created_at: 'DESC' }, take: 10 }),
    ]);
...
- فارسی پاسخ بده - تحلیل داده‌محور بده`;
  ...
  async adminUserChat(targetUserId: string, messages: { role: string; content: string }[]) {
...
فارسی پاسخ بده`;
  ...
  private async callAI(systemPrompt: string, messages: { role: string; content: string }[]) {
  ...
  async streamUserChat(userId: string, messages: { role: string; content: string }[], res: any) {
    const [profile, tests] = await Promise.all([
      this.profileRepo.findOne({ where: { user_id: userId } }),
      this.testRepo.find({ where: { user_id: userId }, order: { completed_at: 'DESC' } }),
    ]);
    ...
    function interpretScore(name: string, s: any): string {
      const toP = (v: any, mx: number) => v!=null?Math.round(Number(v)/mx*100):0;
      if(name==='neo_ffi') return `NEO: E=${s.E||0}/30(${toP(s.E,30)}٪) A=${s.A||0}/30(${toP(s.A,30)}٪) C=${s.C||0}/30(${toP(s.C,30)}٪) N=${s.N||0}/30(${toP(s.N,30)}٪) O=${s.O||0}/30(${toP(s.O,30)}٪)`;
      if(name==='ecr_r') return `ECR: اضطراب=${s.ANX||0}/63(${toP(s.ANX,63)}٪) اجتناب=${s.AVO||0}/63(${toP(s.AVO,63)}٪)`;
      if(name==='erq') return `ERQ: بازارزیابی=${s.CR||0}/42(${toP(s.CR,42)}٪) سرکوب=${s.ES||0}/28(${toP(s.ES,28)}٪)`;
      if(name==='iri') return `IRI: عاطفی=${s.EC||0}/28 شناختی=${s.PT||0}/28`;
      if(name==='gottman') return `گاتمان: ${s.total||0}/11 ${(s.total||0)>=7?'پرخطر':(s.total||0)>=4?'متوسط':'سالم'}`;
      if(name==='phq9') return `PHQ9: ${s.total||0}/27`;
      if(name==='gad7') return `GAD7: ${s.total||0}/21`;
