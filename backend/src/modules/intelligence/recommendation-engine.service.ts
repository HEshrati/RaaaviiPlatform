...
const CAT_TEST_MAP: Record<string, string[]> = {
...
const EVENT_TYPE_EXTRA: Record<string, string[]> = {
...
function shouldRecommend(testName: string, scores: any, forEvent=true): boolean {
  ...
  switch(testName) {
    ...
    case 'gottman': return (Object.values(s).reduce((a:number,b:any)=>a+(Number(b)||0), 0) as number) > 15;
    default:        return true;
  }
}

@Injectable()
export class RecommendationEngineService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    try {
      ...
      `);
      await this.ds.query(
        `CREATE INDEX IF NOT EXISTS idx_smart_rec_user ON smart_recommendations(user_id)`
      );
      this.logger.log('✅ smart_recommendations table ready');
    } catch(e: any) { this.logger.error('Table init error: ' + e.message); }
  }

  // ── فوری بعد از اضافه شدن ایونت ─────────────────────────
  async onNewEvent(eventId: string): Promise<void> {
    ...
    try {
      ...
      const relatedTests = [
        ...
        ...((event.tags || []).flatMap((t: string) => CAT_TEST_MAP[t] || [])),
    ...
    } catch(e: any) { this.logger.error(`event match error: ${e.message}`); }
  }

  // ── فوری بعد از اضافه شدن مقاله ─────────────────────────
  async onNewArticle(articleId: string): Promise<void> {
    this.logger.log(`📚 New article → matching users: ${articleId}`);
    try {
      const [article] = await this.ds.query(
      ...
      let count = 0;
      for (const u of users) {
        if (!shouldRecommend(u.test_name, u.scores, false)) continue;
    ...
    } catch(e: any) { this.logger.error(`article match error: ${e.message}`); }
  }

  // ── sync کامل همه کاربران با همه محتوا ──────────────────
  @Cron('0 */6 * * *') // هر ۶ ساعت
  async fullRecommendationSync(): Promise<void> {
    this.logger.log('🔄 Full recommendation sync...');
    try {
      // ایونت‌های اخیر
      const events = await this.ds.query(
        `SELECT id FROM events WHERE is_active=true AND start_date > NOW() ORDER BY created_at DESC LIMIT 20`
      );
      for (const ev of events) await this.onNewEvent(ev.id);

      // مقالات اخیر
      const articles = await this.ds.query(
        `SELECT id FROM articles WHERE is_published=true ORDER BY created_at DESC LIMIT 30`
      );
      for (const a of articles) await this.onNewArticle(a.id);

      this.logger.log('✅ Full sync complete');
    } catch(e: any) { this.logger.error(`full sync error: ${e.message}`); }
  }

  // ── پیشنهادات کاربر ──────────────────────────────────────
  async getUserRecommendations(userId: string): Promise<{
    events: any[]; articles: any[]; therapists: any[];
  }> {
    ...
    const eventIds = recs.filter((r:any)=>r.type==='event').map((r:any)=>r.item_id);
    const articleIds = recs.filter((r:any)=>r.type==='article').map((r:any)=>r.item_id);

    const [events, articles] = await Promise.all([
      eventIds.length ? this.ds.query(
        `SELECT * FROM events WHERE id = ANY($1) AND is_active=true AND start_date > NOW()`,
        [eventIds]
      ) : [],
      articleIds.length ? this.ds.query(
        `SELECT id,title,category,author,summary,read_time,image_url FROM articles WHERE id::text = ANY($1) AND is_published=true`,
        [articleIds]
      ) : [],
    ]);
  ...
  private calcEventScore(event: any, testName: string, scores: any): number {
    let score = 0.5;
    const s = typeof scores === 'string' ? JSON.parse(scores) : scores || {};
    if (CAT_TEST_MAP[event.category]?.includes(testName)) score += 0.3;
    if (EVENT_TYPE_MAP[event.event_type]?.includes(testName)) score += 0.2;
    if (shouldRecommend(testName, s)) score += 0.1;
    return Math.min(1, score);
  }

  private calcArticleScore(article: any, testName: string, scores: any): number {
    let score = 0.4;
    if (CAT_TEST_MAP[article.category]?.includes(testName)) score += 0.4;
    if (shouldRecommend(testName, scores)) score += 0.2;
    return Math.min(1, score);
  }
}
