...
const ICEBREAKER_BY_NEED: Record<string, string> = {
...
const ARTICLE_TOPICS_BY_NEED: Record<string, string[]> = {
...
@Injectable()
export class RgciService {
  constructor(
    @InjectRepository(RgciResponse)
    private rgciRepo: Repository<RgciResponse>,
    @InjectRepository(RelationalExperienceQuality)
    private reqRepo: Repository<RelationalExperienceQuality>,
    @InjectRepository(PsychologicalOutcome)
    private outcomeRepo: Repository<PsychologicalOutcome>,
    @InjectRepository(ArticleRecommendation)
    private articleRecRepo: Repository<ArticleRecommendation>,
    private dataSource: DataSource,
  ) {}

  getQuestions() {
    return RGCI_QUESTIONS;
  }

  // محاسبه نمره RGCI از پاسخ‌های خام
  calculateRgciScore(rawResponses: Record<string, number[]>): {
    ...
    if (needScore >= 4.5) dominantNeed = 'exit_isolation';
    else if (needScore >= 4.0) dominantNeed = 'being_heard';
    else if (needScore >= 3.5) dominantNeed = 'belonging';
    else if (needScore >= 3.0) dominantNeed = 'deep_talk';
    else if (needScore >= 2.5) dominantNeed = 'fun';
    else dominantNeed = 'growth';

    return { dimensions, total: Math.round(total * 100) / 100, dominantNeed };
  }

  async submitRgci(userId: string, eventId: string | null, rawResponses: Record<string, number[]>) {
  ...
  async getMyRgci(userId: string, eventId?: string) {
  ...
  async submitPostEventSurvey(userId: string, eventId: string, groupId: string | null, rawResponses: Record<string, number>) {
    const dims = [
    ...
    let total = 0;
    const data: any = { user_id: userId, event_id: eventId, group_id: groupId, raw_responses: rawResponses };
    for (const d of dims) {
  ...
  async submitOutcome(userId: string, eventId: string | null, stage: 'pre' | 'post' | 'followup', responses: Record<string, number>) {
    const data = {
      user_id: userId,
      event_id: eventId,
      stage,
    ...
    if (existing) {
      await this.outcomeRepo.update(existing.id, data as any);
      return { ...existing, ...data };
    }
    return this.outcomeRepo.save(this.outcomeRepo.create(data as any));
  }

  async getArticleRecommendations(userId: string) {
  ...
  async getIcebreakerForGroup(dominantNeed: string) {
    return {
      need: dominantNeed,
  ...
  async getFullRgciProfile(userId: string) {
    const rows = await this.dataSource
      ? (this.dataSource as any).query(
          `SELECT * FROM user_rgci_profiles WHERE user_id = $1`, [userId]
        ).catch(() => [])
      : [];

    // اگر dataSource نداشت از repo بخون
    const profile = rows[0] || await (this as any).profileRepo?.findOne({ where: { user_id: userId } });

    if (!profile) return { error: 'پروفایل RGCI یافت نشد.' };

    const level = (score: number | null) => {
