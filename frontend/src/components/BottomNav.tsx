...
const USER_NAV = [
  ...
  { name: "کتابخانه", href: "/articles", icon: BookOpen },
...
const BASE_CLASS =
  "fixed bottom-0 left-0 right-0 z-50 border-t border-slate-700/60 shadow-[0_-4px_32px_rgba(0,0,0,0.3)]";
...
export default function BottomNav() {
  ...
  const wrapperClass = `mobile-safe-bottom-nav ${BASE_CLASS}`;

  if (state.isLoading) {
    return (
      <div className={wrapperClass} style={{ ...NAV_STYLE, height: 68 }} />
    );
  }
  ...
  if (!state.isLoggedIn) {
    return (
      <div className={wrapperClass} style={NAV_STYLE}>
        <div className="flex justify-around items-center h-[68px] w-full px-2 max-w-lg mx-auto">
          {[
            { href: "/", label: "خانه", check: pathname === "/" },
            {
              href: "/events",
              label: "ایونت‌ها",
              check: pathname.startsWith("/events"),
            },
          ].map(({ href, label, check }) => (
            <Link
              key={href}
              href={href}
              className={`flex flex-col items-center justify-center gap-1 w-full h-full relative transition-all duration-300 ${check ? "text-orange-500" : "text-slate-400 hover:text-slate-200"}`}
            >
              {check && <ActiveBar />}
              {href === "/" ? (
                <Home size={check ? 26 : 22} strokeWidth={check ? 2.5 : 2} />
              ) : (
                <Calendar
                  size={check ? 26 : 22}
                  strokeWidth={check ? 2.5 : 2}
                />
              )}
              <span
                className={`text-[10px] ${check ? "font-black" : "font-medium"}`}
              >
                {label}
              </span>
            </Link>
        ...
        {nav.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`flex flex-col items-center justify-center gap-1 w-full h-full relative transition-all duration-300 ${isActive ? "text-orange-500" : "text-slate-400 hover:text-slate-200"}`}
            >
              {isActive && <ActiveBar />}
              <item.icon
                size={isActive ? 26 : 22}
