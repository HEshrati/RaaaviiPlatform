...
const USER_DASH_NAV = [
  ...
  { href: "/hamravan",                     label: "همروان",      icon: Stethoscope },
  { href: "/dashboard/my-therapist/ham-ravan", label: "مشاوره", icon: Stethoscope },
  ...
  { href: "/dashboard/sessions",           label: "جلسات",       icon: CalendarCheck },
...
const ADMIN_DASH_NAV = [
  ...
  { href: "/admin/professionals", label: "مدیریت حرفه‌ای‌ها", icon: ClipboardCheck },
  { href: "/admin/crm", label: "CRM", icon: Cpu },
...
export default function TopHeader() {
  ...
  const navItems = inAdminDashboard ? ADMIN_DASH_NAV : USER_DASH_NAV;

  // ─── نوبار داشبورد ───────────────────────────────────────────
  if (inDashboard) {
    return (
      <header
        className="mobile-safe-topbar md:hidden fixed top-0 left-0 right-0 z-50 flex items-center h-14"
        style={{
          background: "#090e1c",
          borderBottom: "none",
        }}
                ...
                >
                  ...
                  {active && (
                    <span
                      ...
                      style={{
                        background: inAdminDashboard ? "#818cf8" : "#FF6B00",
                      }}
                    />
                  )}
                </Link>
  ...
  return (
    <header
      className="mobile-safe-header fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 h-16"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-2">
        {/* دکمه ورود/ثبت‌نام برای کاربران وارد نشده */}
        {!isLoggedIn && (
          <Link
            href="/login"
            className="flex items-center gap-1.5 rounded-full px-4 py-1.5 text-sm font-black transition-all active:scale-95"
            style={{
        ...
        {isLoggedIn && (
          <Link
            ...
            style={{
              ...
              border: "1.5px solid rgba(26,58,92,0.1)",
            }}
          >
            <Bell size={18} style={{ color: "#1a3a5c" }} />
          </Link>
        )}

        {/* زنگوله اعلان‌ها */}

        {/* دکمه شهر — فقط برای لاگین‌شده‌ها */}
        {isLoggedIn && (
          <button
            onClick={() => router.push("/dashboard/profile")}
