...
export default function MyTherapistLandingPage() {
  ...
  const { user } = useApp();
  // نقش‌های حرفه‌ای نباید برای دسترسی به هم‌روان/هم‌زیسته نیاز به تست یا پرسش‌نامه داشته باشند
  ...
  // sync تست‌ها برای فیلتر روانشناسان

  // sync بعد از تست — پیشنهاد روانشناس آپدیت میشه
  useEffect(()=>{
    try {
  ...
  useEffect(() => {
    ...
    fetch("https://raaviiplatform.com/api/test-results/my", {
      ...
      .then((d:any) => { const _r=(d?.results||d?.data||[]); (_r); })
      .catch(() => {});
  }, []);

  useEffect(() => {
    // بایپس تست برای حساب‌های حرفه‌ای (روانشناس/تسهیلگر/ادمین)
    if (isProfessional) { setIntakeDone(true); return; }
            ...
            <div className="flex gap-1">
              {[1,2,3,4,5].map(i => (
                <div key={i} className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black"
                  style={{ 
                    background: i <= coreTestsDone ? "linear-gradient(135deg,#FF6B00,#f97316)" : "rgba(0,0,0,0.08)",
                    color: i <= coreTestsDone ? "white" : "#94a3b8"
                  }}>
                  {i <= coreTestsDone ? "✓" : i}
                </div>
              ))}
            </div>
            <span className="text-sm font-black" style={{ color: coreTestsDone>=5?"#22c55e":"#FF6B00" }}>
              {coreTestsDone} از ۵ تست اصلی
            </span>
          </div>
          {coreTestsDone < 5 && (
            <a href="/dashboard/tests" className="text-xs font-bold text-orange-500">تکمیل →</a>
          )}
        </div>
        )}

        {!isProfessional && intakeDone === false && (
          <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          ...
          >
            ...
            >
              ...
              <div
                className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-[10px] font-black"
                style={{ background: "rgba(255,255,255,0.95)", color: "#10b981" }}
              >
                گروهی
              </div>
            </div>
