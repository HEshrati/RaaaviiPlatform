...
function safeUpdateContext(app: any, partial: Record<string, any>) {
  try {
    ...
    else if (typeof app.dispatch === "function") app.dispatch({ type: "SET_USER", payload: newUser });
  } catch {}
}
...
export default function PersonalityTest() {
  ...
  useEffect(() => {
    ...
    const taken = app?.state?.user?.isTestTaken || localUser?.isTestTaken;

    if (taken) {
      fetch("/api/auth/refresh",{method:"POST",credentials:"include"}).catch(()=>{});
      router.replace("/dashboard");
    }
  }, [app?.state?.user?.isTestTaken, router]);

  const scores = useMemo(
    () => Object.values(answers).reduce((acc, option) => {
  ...
  const handleSelectOption = (questionId: number, option: Option) => {
    ...
    setAnswers(next);
    if (!isLastQuestion) {
      setTimeout(() => setCurrentQuestion((q) => Math.min(q + 1, totalQuestions - 1)), 250);
    }
  };

  const goBack = () => { setError(null); setCurrentQuestion((q) => Math.max(0, q - 1)); };
  ...
  const finishTest = async () => {
    if (saving) return;
    if (!allAnswered) { setError("لطفاً به همه سوالات پاسخ بده."); return; }

    setSaving(true);
    setError(null);
    setSaveFailed(false);

    const finalScores = Object.values(answers).reduce((acc, item) => {
      Object.entries(item.scores).forEach(([axis, value]) => { acc[axis as Axis] += value || 0; });
      return acc;
    }, { ...emptyScores });
    const finalResult = buildResult(finalScores);
    const answersMap = Object.fromEntries(Object.entries(answers).map(([k, v]) => [k, v.id]));

    // ✅ اول isTestTaken رو ست کن تا gate ها ریدایرکت نکنن
    ...
    try {
      ...
      await withTimeout(
        saveTestResult({
          ...
          scores: { ...finalScores, fullType: finalResult.type, matchingBasis: finalResult.matchingBasis, answers: answersMap },
        }),
        8000,
      );
      // ✅ backend رو هم آپدیت کن
      const token = typeof window !== "undefined" ? localStorage.getItem("token") : null;
      if (token) {
        fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/auth/mark-test-taken`, {
      ...
      >
        <div className="w-full max-w-sm rounded-3xl p-6 shadow-2xl bg-white">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-black text-slate-900 text-base">خروج از تست</h3>
              <p className="text-slate-500 text-xs mt-0.5">پاسخ‌های فعلی ذخیره نمی‌شن</p>
            </div>
          </div>
          <p className="text-slate-600 text-sm leading-relaxed mb-5">
            مطمئنی می‌خوای از تست خارج بشی؟ برای مچینگ بهتر، بهتره تست رو کامل کنی.
          </p>
    ...
    return (
      ...
      >
        <div className="max-w-2xl w-full bg-white rounded-3xl shadow-2xl p-8 text-center border border-slate-200">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-1">{result.code}</h2>
          <p className="text-sm text-slate-600 font-bold mb-3">{result.type}</p>
          <p className="text-base text-slate-600 leading-8 mb-6">{result.description}</p>
          ...
          {saveFailed && (
            <div className="rounded-2xl p-3 mb-5 flex items-start gap-2 text-right border bg-yellow-50 border-yellow-200">
              ...
              <p className="text-xs text-yellow-800">
                نتیجه روی این دستگاه ذخیره شد، اما به سرور نرسید. ممکنه به‌خاطر اتصال اینترنت باشه.
              </p>
            </div>
          )}
      ...
      <div className="max-w-2xl w-full">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2 text-sm">
            <span className="text-slate-600">سوال {currentQuestion + 1} از {totalQuestions}</span>
            <span className="font-black" style={{ color: ORANGE }}>{progressPercent}٪</span>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 border border-slate-200">
          ...
          <h2 className="text-xl md:text-2xl font-black text-slate-900 leading-9">{question.text}</h2>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    {isSelected && <CheckCircle2 size={20} className="text-white flex-shrink-0" />}
                  </div>
                </button>
              );
            })}
            ...
            {isLastQuestion && (
              <button
                ...
                style={{
                  ...
                  boxShadow: saving || !currentAnswered ? "none" : NAVY_BTN_SHADOW,
                }}
              >
                {saving ? (
                  <>
